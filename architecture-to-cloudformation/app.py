...
st.set_page_config(
    page_title="Cloud Infrastructure",
    page_icon="☁️",
    layout="wide",
)
...
st.header("Architecture to Infrastructure as Code")

# Using object notation

st.sidebar.header("Inference Parameters")
Temperature = st.sidebar.slider("Temperature", min_value=0.0, max_value=1.0, step=0.1, value=0.0)
...
# Add radio buttons for cloud provider and infrastructure as code tool
cloud_provider = st.sidebar.radio("Cloud Provider", ["AWS", "Azure"])
iac_tool = st.sidebar.radio("Infrastructure as Code Tool", ["CloudFormation", "Terraform"])
...
uploaded_file = st.file_uploader(
    f"Upload an Architecture diagram to generate {cloud_provider} {iac_tool} code",
    type=["jpeg", "png"],
...
if uploaded_file is not None:
    ...
    if prompt := st.chat_input("Give the bot instructions to update the infrastructure..."):
